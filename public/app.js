...
async function loginPage() {
  ...
  const [me, auth] = await Promise.all([fetch('/api/me').then((r) => r.ok ? r.json() : null), fetch('/api/auth/status').then((r) => r.json())]);
  if (!auth.enabled) { document.querySelector('#account-content').innerHTML = `<div class="account">Der Account-Bereich ist vorbereitet. Der öffentliche MCP-Endpunkt ist bereits nutzbar.</div><p class="form-note">Die Anmeldung wird aktiviert, sobald die produktive D1-Datenbank freigeschaltet ist.</p>`; return; }
